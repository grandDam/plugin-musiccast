...
<div class="row row-overflow">
        ...
        <div class="bs-sidebar">
            <ul id="ul_eqLogic" class="nav nav-list bs-sidenav">
                <a class="btn btn-default eqLogicAction" style="width : 100%;margin-top : 5px;margin-bottom: 5px;"
                   data-action="add"><i class="fa fa-plus-circle"></i> {{Ajouter un template}}</a>
    ...
    <div class="col-lg-10 col-md-9 col-sm-8 eqLogicThumbnailDisplay"
        ...
        <div class="eqLogicThumbnailContainer">
            <legend><i class="fa fa-table"></i> {{Mes MusicCast}}</legend>
            <?php
            foreach ($eqLogics as $eqLogic) {
                $opacity = ($eqLogic->getIsEnable()) ? '' : jeedom::getConfiguration('eqLogic:style:noactive');
                echo '<div class="eqLogicDisplayCard cursor" data-eqLogic_id="' . $eqLogic->getId() . '" style="text-align: center; background-color : #ffffff; height : 200px;margin-bottom : 10px;padding : 5px;border-radius: 2px;width : 160px;margin-left : 10px;' . $opacity . '" >';
                if ($eqLogic->getConfiguration('model', '') != '') {
                    echo '<img src="plugins/musiccast/core/img/' . $eqLogic->getConfiguration('model', '') . '.jpg" height="105" width="95" />';
                } else {
                        ...
                        <form class="form-horizontal">
                            <fieldset>
                                <div class="form-group">
                                    <label class="col-sm-3 control-label">{{Nom du MusicCast}}</label>
                                    <div class="col-sm-5">
...
<?php include_file('desktop', 'musiccast', 'js', 'musiccast'); ?>
